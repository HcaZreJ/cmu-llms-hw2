...
class ASCIIBPETokenizer:
    ...
    def merge(self, token_ids: list[int]) -> list[int]:
        ...
        """
        # The new token_id T just need to be equal to the vocab size
        T = len(self.vocab)
        
        # Compute bigram statistics using the provided algorithm
        bigram_count = compute_bigram_statistics(token_ids)

        # First find the highest bigram frequency, in case there are ties
        most_frequent_frequency = bigram_count.most_common(1)[0][1]

        # Get all bigrams with that frequency in case there are multiple ties
        most_frequent_bigrams = [bigram for bigram, count in bigram_count.items() if count == most_frequent_frequency]

        # Find the lexicographically smallest, most frequent bigram
        smallest_most_frequent_bigram = min(most_frequent_bigrams)

        # Add this bigram to vocab, additionally update merge rule
        self.vocab.append("".join([self.vocab[token] for token in smallest_most_frequent_bigram]))
        self.merge_rules[smallest_most_frequent_bigram] = T

        # Replace the lexicographically smallest, most frequent bigram using the provided algorithm, then return
        new_token_ids = replace_bigram(token_ids, smallest_most_frequent_bigram, T)
        return new_token_ids
    ...
    def encode(self, text: str) -> list[int]:
        ...
        # First convert text to ASCII in the range of 0-127, then merge bigrams based on merge rules.
        token_ids = [ord(char) for char in text]

        for bigram, bigram_token in self.merge_rules.items():
            token_ids = replace_bigram(token_ids, bigram, bigram_token)
