...
def random_batch_sampler(
    ...
    while True:
        max_starting_idx = len(tokens) - seq_len + 1
        # Randomly select a start index for each batch
        start_indices = torch.randint(0, max_starting_idx, (batch_size,))
        # Obtain the batch training data of (B * S)
        batch = torch.stack([tokens[i:i + seq_len] for i in start_indices])
...
def sequential_batch_sampler(
    ...
    Note: If the last batch is incomplete, which could happen when the number
        ...
        the last batch.
    """
    # Calculate cutoff of tokens that we will iterate through in case last batch is incomplete
    cutoff = (len(tokens) // (batch_size * seq_len)) * (batch_size * seq_len)

    # Reshape tokens, this is most efficient because nothing is actually done on the tensor
    reshaped_tokens = tokens[:cutoff].view(-1, batch_size, seq_len)

    for batch in reshaped_tokens:
        yield batch.to(device)
...
def cosine_lr_schedule(
    ...
    def get_lr(t: int) -> float:
        ...
        if t <= num_warmup_steps:
            ...
            cosine_decay = 0.5 * (1 + math.cos(math.pi * (t - num_warmup_steps) / (num_training_steps - num_warmup_steps)))
...
def compute_language_modeling_loss(
    ...
    """
    # For every sequence, the ground truth of this token's next token is the next token
    labels = input_ids[:, 1:].contiguous().view(-1)
    # input_ids has shape (B, S), labels will have shape (B * (S-1))

    # Drop every sequence's last token because there's nothing to predict
    logits = logits[:, :-1].contiguous().view(-1, logits.size(-1))
    # logits after reshape will have shape (B * (S-1), n_vocab)
