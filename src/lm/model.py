...
class MultiHeadAttention(nn.Module):
    ...
    def self_attention(
        ...
        """
        causal_mask = torch.tril(torch.ones(qkT.size(2), qkT.size(2))).to(q.device).bool()
        ...
        attn = rearrange(torch.matmul(attn_weights, v), "b h s hd -> b s (h hd)")
...
class DecoderLM(nn.Module):
    ...
    def embed(
        ...
        positional_embeddings = self.position_embeddings(position_ids.clamp(min=0, max=self.n_positions - 1))
